...
precision  <- round(
  training_metrics$value[training_metrics$metric == "precision"],
  4
)
auc_val <- 0.811
...
`r accuracy * 100`% and precision of `r precision*100`%. One of the patterns we found from our exploratory data analysis was
...
`income`. The model achieved an overall accuracy of `r accuracy * 100`% and precision of `r precision*100`%.
